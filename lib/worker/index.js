...
async function _sync(session) {
  ...
  try {
    ...
    while(!halt()) {
      ...
      logger.debug('after gossipCycle', {ledgerNodeId});
      // work session expired
      if(halt()) {
        break;
      }
      ...
      logger.debug('after eventWriter', {ledgerNodeId});
      // 4. merge if possible
      const {merged} = await merge(
        {ledgerNode, creatorId, priorityPeers, halt});
      logger.debug('after merge', {ledgerNodeId});
