...
module.exports = class GossipPeerSelector {
  ...
  async getPeers({priorityPeers}) {
    ...
    logger.debug(
      `${this.creatorId}: priorityPeers count: ${priorityPeers.length}`);
    const candidates = _.shuffle(priorityPeers.filter(p => p !== creatorId));
    logger.debug(
      `${this.creatorId}: canidates count: ${candidates.length}`);
  ...
  async _filterPeers({candidates, priorityPeers, coolDownPeriod = 0}) {
    ...
    const filtered = [];
    const now = Date.now();
    logger.debug(`${this.creatorId}: filtering peers`);
    for(const creatorId of candidates) {
      ...
      logger.debug(
        `${this.creatorId}: peer info: ` +
        `${JSON.stringify({backoff, lastContactDate})}`);
    ...
    logger.debug(`${this.creatorId}: filtered peer count: ${filtered.length}`);
    return filtered;
  }
};
