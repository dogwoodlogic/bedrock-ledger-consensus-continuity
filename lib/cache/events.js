 ...
 */
api.getRecentHistory = async ({ledgerNode}) => {
  ...
  if(result.includes(null)) {
    logger.debug('One or more events are missing from the cache, ' +
      'Attempting recovery...');
    const missingHashes = [];
    for(let i = 0; i < result.length; ++i) {
      if(result[i] === null) {
        const key = keys[i];
        const eventHash = key.substr(key.lastIndexOf('|') + 1);
        let record;
        try {
          // the get API throws NotFoundError
          record = await ledgerNode.storage.events.get(eventHash);
        } catch(error) {
          if(error.name !== 'NotFoundError') {
            throw new BedrockError(
              'One or more events are missing from the cache. An error ' +
              'occurred while attempting to recover an event.',
              'InvalidStateError', {
                eventHash,
                httpStatusCode: 400,
                public: true,
          ...
          missingHashes.push(eventHash);
          continue;
        }
        const eventSummary = JSON.stringify({
          event: {
            parentHash: record.event.parentHash,
            treeHash: record.event.treeHash,
            type: record.event.type
          },
          meta: {
            eventHash: record.meta.eventHash,
            continuity2017: {
              creator: record.meta.continuity2017.creator
            }
          }
        });
        result[i] = eventSummary;
        await cache.client.set(key, eventSummary);
        logger.debug('Found missing event.', {eventHash, record});
      }
    }
    if(missingHashes.length > 0) {
      logger.debug('Event recovery failed.', {missingHashes});
      throw new BedrockError(
        'One or more events are missing from the cache and could not be ' +
        'recovered.', 'InvalidStateError', {
          httpStatusCode: 400,
          missingHashes,
          public: true,
        });
    }
    logger.debug('Event recovery successful.');
  }
