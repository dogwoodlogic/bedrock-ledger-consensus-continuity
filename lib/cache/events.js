  ...
  if(result.includes(null)) {
    ...
    for(let i = 0; i < result.length; ++i) {
      if(result[i] === null) {
        ...
        const event = await ledgerNode.storage.events.get(eventHash);
        logger.debug('FOUND MISSING EVENT', {eventHash, event});
        missingHashes.push(keys[i]);
      }
    }
    logger.debug('INVALID_STATE_ERROR MISSING HASHES', {missingHashes});
